    ...
    try {
      ...
      const records = truthRows.flatMap((tr): any[] => {
        ...
        return [{
          ...
          regular_hours: 0,
          overtime_hours: 0,
          // ⚠️ HARD PAYROLL RULE: In truth-based closure, truth file is the authority.
          // System amounts are diagnostic only — never use them as closure totals.
          hourly_pay_total: truthTotal > 0 ? truthTotal : (Number(tr.system_total_pay) || 0),
          daily_pay_total: Number(tr.truth_pay_per_day) || Number(tr.system_pay_per_day) || 0,
          ride_pay_total: Number(tr.truth_ryde) || Number(tr.system_ryde) || 0,
          weekend_pay_total: 0,
          manual_adjustment_total: Number(tr.truth_tips) || Number(tr.system_tips) || 0,
          // grand_total and final_total_pay = TRUTH amount (authoritative for closure)
          grand_total: truthTotal,
          ride_amount: Number(tr.truth_ryde) || Number(tr.system_ryde) || 0,
          weekend_amount: 0,
          manual_amount: Number(tr.truth_tips) || Number(tr.system_tips) || 0,
          base_pay: truthTotal > 0 ? truthTotal : (Number(tr.system_total_pay) || 0),
          ...
          warnings: systemTotal > truthTotal + 1 ? [`Sistema interno ($${systemTotal}) excede truth pagado ($${truthTotal}) — truth es autoritativo`] : [],
