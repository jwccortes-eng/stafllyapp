...
export function parseTruthFile(data: ArrayBuffer | Uint8Array): TruthParseResult {
  ...
  const cols = {
    ...
    total_hours: findCol(headers, ["total paid hours", "total work hours", "total hours", "hours", "horas", "horas pagadas"]),
    ...
    travel_hours: findCol(headers, ["travel hours", "horas de viaje", "travel"]),
    otros: findCol(headers, ["otros", "other", "others"]),
    discount: findCol(headers, ["discount", "discount ", "descuento", "descuentos"]),
  ...
  for (let i = 1; i < raw2d.length; i++) {
    ...
    const truthRow: TruthRow = {
      ...
      travel_hours: parseMoney(cols.travel_hours >= 0 ? row[cols.travel_hours] : null),
      otros: parseMoney(cols.otros >= 0 ? row[cols.otros] : null),
      discount: parseMoney(cols.discount >= 0 ? row[cols.discount] : null),
