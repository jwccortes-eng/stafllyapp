...
function buildLocationPopup(location: LiveMapLocation): string {
  ...
  return `<div style="font-size:12px;line-height:1.35;min-width:160px;">
...
function buildWorkerPopup(worker: LiveMapWorker): string {
  ...
  const elapsed = Math.round((Date.now() - new Date(worker.clock_in).getTime()) / 60000);
  ...
  return `<div style="font-size:12px;line-height:1.35;min-width:200px;">
    ...
    <p style="margin:0 0 3px;"><strong>Tiempo activo:</strong> ${elapsedStr}</p>
    <p style="margin:0 0 6px;"><strong>GPS:</strong> ±${Math.round(worker.accuracy)}m</p>
...
export function LiveOperationsMap({ workers, locations, showLayer }: LiveOperationsMapProps) {
  ...
  useEffect(() => {
    ...
    }).setView([39.8283, -98.5795], 4); // US center as initial — will fitBounds immediately
  ...
  useEffect(() => {
    ...
    // If no data, don't recenter — keep the previous view or US center
