...
// Convert string to ArrayBuffer for the async API
...
describe("parseConnecteamFile", () => {
  it("should parse semicolon-delimited CSV from Connecteam export", async () => {
    const result = await parseConnecteamFile(strToArrayBuffer(SAMPLE_CSV), "users_4.csv");
  ...
  it("should extract name, phone, email, and connecteam_employee_id correctly", async () => {
    const result = await parseConnecteamFile(strToArrayBuffer(SAMPLE_CSV), "users_4.csv");
