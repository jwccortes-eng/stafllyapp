...
DROP TRIGGER IF EXISTS trg_bump_shift_assignment_version ON public.shift_assignments;
CREATE TRIGGER trg_bump_shift_assignment_version
  BEFORE UPDATE ON public.shift_assignments
  FOR EACH ROW EXECUTE FUNCTION public.bump_row_version();

-- Carril único de transición de estado de asignaciones.
...
BEGIN
  ...
  -- Idempotencia: mismo intent_key ya aplicado → replay sin efectos nuevos.
  IF p_intent_key IS NOT NULL THEN
    SELECT after_values INTO v_replay
      FROM public.versioned_write_audit
     WHERE entity = 'shift_assignments'
       AND intent_key = p_intent_key
       AND result = 'applied'
  ...
  SELECT * INTO v_a FROM public.shift_assignments
  ...
  -- Fail-closed: versión o estado esperado desactualizado.
  IF (p_expected_version IS NOT NULL AND p_expected_version IS DISTINCT FROM v_a.version)
    ...
    VALUES ('shift_assignments', v_a.id, v_a.company_id, v_uid, p_expected_version, v_a.version,
      CASE WHEN p_expected_status IS DISTINCT FROM v_a.status AND p_expected_status IS NOT NULL
  ...
  BEGIN
    IF p_transition = 'remove' THEN
      ...
      UPDATE public.shift_assignments
         SET removed_by = v_uid, removed_at = now()
       WHERE id = v_a.id;
    ...
    ELSIF p_transition IN ('set_role_driver','set_role_worker') THEN
      IF v_a.status IN ('removed','rejected') THEN
        RAISE EXCEPTION 'assignment_inactive' USING ERRCODE = '22023';
      END IF;
      UPDATE public.shift_assignments
         SET assignment_role = CASE WHEN p_transition = 'set_role_driver' THEN 'driver' ELSE 'worker' END
       WHERE id = v_a.id;
      ...
      v_driver_impact := CASE WHEN v_next_driver IS NULL THEN 'no_driver_left'
                              WHEN p_transition = 'set_role_driver' THEN 'promoted' ELSE 'demoted' END;

    ELSIF p_transition = 'set_captain' THEN
      IF v_a.status IN ('removed','rejected') THEN
        RAISE EXCEPTION 'assignment_inactive' USING ERRCODE = '22023';
      END IF;
      UPDATE public.scheduled_shifts SET shift_admin_id = v_a.employee_id WHERE id = v_a.shift_id;
      -- Toca la fila para versionar el cambio de responsabilidad.
      UPDATE public.shift_assignments SET assignment_role = COALESCE(assignment_role,'worker') WHERE id = v_a.id;
    ...
    ELSIF p_transition = 'replace' THEN
      ...
      IF NOT FOUND THEN
        v_new_assignment := public.assign_worker_to_shift(
          v_a.shift_id, p_target_employee_id, COALESCE(v_a.assignment_role,'worker'),
      ...
      END IF;
      UPDATE public.shift_assignments
         SET status = 'replaced', removed_by = v_uid, removed_at = now(),
             replaced_by_assignment_id = v_new_assignment.id
       WHERE id = v_a.id;
      v_driver_impact  := COALESCE(v_delegated->>'driver_impact','none');
      v_captain_impact := COALESCE(v_delegated->>'captain_impact','none');
  ...
  VALUES ('shift_assignments', v_after.id, v_after.company_id, v_uid, p_expected_version, v_after.version,
    NULL, ARRAY[p_transition], p_surface, p_intent_key, 'applied', p_reason,
    v_before, v_result);
...
REVOKE ALL ON FUNCTION public.versioned_assignment_transition(uuid,uuid,text,text,integer,text,uuid,text,text) FROM public;
GRANT EXECUTE ON FUNCTION public.versioned_assignment_transition(uuid,uuid,text,text,integer,text,uuid,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.versioned_assignment_transition(uuid,uuid,text,text,integer,text,uuid,text,text) TO service_role;