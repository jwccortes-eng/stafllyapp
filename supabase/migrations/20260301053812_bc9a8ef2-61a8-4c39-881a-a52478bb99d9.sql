...
-- 1. Ensure an "Hourly Rate" concept exists for each company
INSERT INTO concepts (company_id, name, category, calc_mode, rate_source, unit_label, is_active)
SELECT DISTINCT c.id, 'Hourly Rate', 'extra'::concept_category, 'quantity_x_rate'::calc_mode, 'per_employee'::rate_source, 'hrs', true
FROM companies c
WHERE NOT EXISTS (
  SELECT 1 FROM concepts co WHERE co.company_id = c.id AND co.name = 'Hourly Rate'
);

-- 2. Recreate the consolidation function with rate fallback
...
BEGIN
  ...
  IF _clock_source = 'time_entries' THEN
    WITH clock_hours AS (
      SELECT te.employee_id,
        SUM(EXTRACT(EPOCH FROM (te.clock_out::timestamptz - te.clock_in::timestamptz)) / 3600.0
          - COALESCE(te.break_minutes, 0) / 60.0) AS total_work_hours
      FROM time_entries te
    ...
    emp_rates AS (
      ...
      FROM clock_hours ch WHERE ch.total_work_hours > 0
    ),
    calculated AS (
  ...
  ELSE
    WITH clock_hours AS (
      SELECT s.employee_id,
        SUM(EXTRACT(EPOCH FROM (s.clock_out_time::timestamptz - s.clock_in_time::timestamptz)) / 3600.0) AS total_work_hours,
        COALESCE(AVG(NULLIF(s.hourly_rate_usd, 0)), 0) AS avg_rate
      FROM shifts s
      WHERE s.company_id = _company_id AND s.period_id = _period_id
        AND s.clock_in_time IS NOT NULL AND s.clock_out_time IS NOT NULL
    ...
    calculated AS (
      SELECT ch.employee_id,
        ...
        ch.total_work_hours AS total_paid_hours, ch.avg_rate,
        ROUND((LEAST(ch.total_work_hours, _ot_threshold) * ch.avg_rate
           + GREATEST(ch.total_work_hours - _ot_threshold, 0) * ch.avg_rate * 1.5)::numeric, 2) AS base_total_pay
      FROM clock_hours ch
    )
    INSERT INTO period_base_pay (company_id, period_id, employee_id, total_work_hours, total_regular, total_overtime, total_paid_hours, base_total_pay, import_id)
    SELECT _company_id, _period_id, c.employee_id, c.total_work_hours, c.total_regular, c.total_overtime, c.total_paid_hours, c.base_total_pay, NULL
  ...
  SELECT id INTO _daily_concept_id
    FROM concepts
   WHERE company_id = _company_id AND name = 'Daily Pay' AND category = 'extra'
   LIMIT 1;

  IF _daily_concept_id IS NULL THEN
    INSERT INTO concepts (company_id, name, category, calc_mode, rate_source, unit_label, is_active)
    VALUES (_company_id, 'Daily Pay', 'extra', 'quantity_x_rate', 'per_employee', 'días', true)
  ...
  WITH daily_assignments AS (
    SELECT sa.employee_id,
      ...
      COALESCE(
        (SELECT cer.rate FROM concept_employee_rates cer
          WHERE cer.concept_id = _daily_concept_id AND cer.employee_id = sa.employee_id
            AND (cer.effective_from IS NULL OR cer.effective_from <= _period.end_date::date)
            AND (cer.effective_to IS NULL OR cer.effective_to >= _period.start_date::date)
          ORDER BY cer.effective_from DESC NULLS LAST LIMIT 1),
    ...
    WHERE ss.company_id = _company_id AND ss.pay_type = 'daily' AND ss.deleted_at IS NULL
      AND ss.date >= _period.start_date::date AND ss.date <= _period.end_date::date
  ...
  SELECT _company_id, _period_id, da.employee_id, _daily_concept_id,
    da.days_worked, da.daily_rate, da.days_worked * da.daily_rate,
    'Auto: ' || da.days_worked || ' día(s) de turno diario', auth.uid()
  FROM daily_assignments da
  WHERE da.daily_rate > 0
    AND NOT EXISTS (
      SELECT 1 FROM movements m WHERE m.period_id = _period_id AND m.employee_id = da.employee_id AND m.concept_id = _daily_concept_id AND m.company_id = _company_id
    );

  GET DIAGNOSTICS _daily_movements = ROW_COUNT;

  _result := jsonb_build_object(
    'success', true, 'consolidated_employees', _affected,
    'skipped_import_employees', _skipped_import, 'ot_threshold', _ot_threshold,
    'period_id', _period_id::text, 'clock_source', _clock_source,
    'open_entries', _open_entries, 'rejected_entries', _rejected_entries,
    'daily_pay_movements', _daily_movements
  );

  INSERT INTO activity_log (user_id, company_id, action, entity_type, entity_id, details)
  VALUES (auth.uid(), _company_id, 'consolidate_clock', 'pay_period', _period_id::text, _result);
