...
BEGIN
  ...
  IF _clock_source = 'time_entries' THEN
    WITH clock_hours AS (
      ...
      LEFT JOIN shifts s ON s.employee_id = te.employee_id AND s.period_id = _period_id AND s.company_id = _company_id
  ...
  IF _daily_concept_id IS NULL THEN
    ...
    VALUES (_company_id, 'Daily Pay', 'extra', 'qty_x_rate', 'employee_rate', 'días', true)
